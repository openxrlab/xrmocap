...
class PredictionMatcher(BaseMetric):
    """Prediction matcher matches the predictions with ground truth
    when the number of prediction does not align with 
    the number of ground truth, this metric will return pred2gt 
    match and gt2pred matching for various metric usage.
    ...
    def __call__(self, pred_keypoints3d: Keypoints, gt_keypoints3d: Keypoints,
        ...
        if self.align_kps_name is None:
            ...
            gt_kps3d = align_by_keypoint(gt_keypoints3d, self.align_kps_name)[..., :3]
        ...
        for frame_idx in range(n_frame):
            dist[frame_idx, :] = vectorize_distance(
                gt_kps3d[frame_idx, :], pred_kps3d[frame_idx, :])
        ...
        return dict(match_matrix_gt2pred=match_matrix_gt2pred,
