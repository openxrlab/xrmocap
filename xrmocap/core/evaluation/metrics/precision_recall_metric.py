...
class PrecisionRecallMetric(BaseMetric):
    ...
    def __call__(self, pred_keypoints3d: Keypoints, 
                 gt_keypoints3d: Keypoints, **kwargs):
        ...
        pred_n_frame, pred_n_person = pred_keypoints3d.get_keypoints().shape[:2]
        ...
        tb, precision_recall_dict = self.evaluate_map(pred_keypoints3d, gt_keypoints3d )
        if self.show_table:
    ...
    def evaluate_map(
        ...
        for frame_idx in range(self.n_frame):
            for person_idx in range(gt_n_person):
                gt_person_mask = gt_keypoints3d.get_mask()[frame_idx, person_idx, ...]
            ...
            for person_idx in range(pred_n_person):
                person_mask = pred_keypoints3d.get_mask()[frame_idx,person_idx, :]
                person_score = pred_keypoints3d.get_keypoints()[frame_idx, person_idx, :, -1]\
                    [np.where(person_mask>0)].mean()
                person_mpjpe = self.mpjpe_value_pred2gt[frame_idx, person_idx][np.where(person_mask>0)].mean()
                gt_id_acc = frame_idx * gt_n_person + self.match_matrix_pred2gt[frame_idx, person_idx]
        ...
        tb.add_row(['AP'] + [f'{aps[i] * 100:.2f}' for i in range(len(self.threshold))])
        tb.add_row(['Recall'] + [f'{recs[i] * 100:.2f}' for i in range(len(self.threshold))])
        
        precision_recall_dict = {}
        for i in range(len(self.threshold)):
            precision_recall_dict[f'recall@{self.threshold[i]}'] = recs[i] * 100
