...
class End2EndEvaluation(BaseEvaluation):
    ...
    def __init__(
        ...
        """Evaluation for end-to-end methods. For model validation during training
    ...
    def run(self, 
        ...
        if is_main_process():
            ...
            if self.dataset_name == 'panoptic' and self.trans_matrix is not None:
                for frame_idx in range(self.n_frame):
                    for person_idx in range(gt_n_person):
                        gt_kps3d[frame_idx, person_idx, :, :3] = \
                            gt_kps3d[frame_idx, person_idx, :, :3].dot(self.trans_matrix)
            ...
            # convert pred and gt to the same convention
            if gt_n_kps > pred_n_kps:
                gt_keypoints3d_raw= convert_keypoints(
                    gt_keypoints3d_raw,
                    dst=self.pred_kps3d_convention,
                    approximate=True)
            elif gt_n_kps < pred_n_kps:
                pred_keypoints3d_raw= convert_keypoints(
            ...
            if not is_train:
                pred_file_path = os.path.join(self.output_dir, 'pred_keypoints3d.npz')
                gt_file_path = os.path.join(self.output_dir, 'gt_keypoints3d.npz')
                ...
                if self.dataset_visualization is not None:
                    self.dataset_visualization.pred_kps3d_paths = pred_file_path
