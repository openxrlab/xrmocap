...
from xrmocap.ops.parametric_optimization.base_optimization import BaseOptimization


class SkelInfo():
    def __init__(self,kps_convention) -> None:
        self.kps_convention = kps_convention
        self.boneLen = np.zeros(skel_info[self.kps_convention]['n_kps'] - 1,dtype=np.float32)
        self.boneCnt = np.zeros(skel_info[self.kps_convention]['n_kps'] - 1,dtype=np.float32)
        self.active = 0.0
        self.shapeFixed = False
        self.data = np.zeros(3+skel_info[self.kps_convention]['n_kps']*3+skel_info[self.kps_convention]['shape_size'],dtype=np.float32)
...
class SkelSolver():
    def __init__(self,kps_convention) -> None:
        self.kps_convention = kps_convention
        self.m_joints = np.array(skel_info[self.kps_convention]['m_joints']).reshape(3,skel_info[self.kps_convention]['n_kps'])
        self.m_jShapeBlend = np.array(skel_info[self.kps_convention]['m_jShapeBlend']).reshape(skel_info[self.kps_convention]['n_kps']*3, skel_info[self.kps_convention]['shape_size'])
        self.m_boneShapeBlend= np.zeros((3 * (skel_info[self.kps_convention]['n_kps'] - 1),  skel_info[self.kps_convention]['shape_size']), dtype=np.float32)
    ...
    def CalcNodeWarps(self, param, jBlend):
        ...
        for jIdx in range(jBlend.shape[1]):
            ...
            if jIdx == 0:
                ...
                matrix[:3,-1:] = (jBlend[:,jIdx] - jBlend[:,skel_info[self.kps_convention]['joint_parent'][jIdx]]).reshape((-1,1)) 
    ...
    def CalcChainWarps(self,nodeWarps):
        ...
        for jIdx in range(int(nodeWarps.shape[1]/4)):
            if jIdx == 0:
                ...
                chainWarps[:,jIdx * 4:jIdx * 4+4] = np.matmul(chainWarps[:,skel_info[self.kps_convention]['joint_parent'][jIdx] * 4:skel_info[self.kps_convention]['joint_parent'][jIdx] * 4+4], nodeWarps[:,jIdx * 4:jIdx * 4+4])
        return chainWarps
    ...
    def AlignRT(self,term,param):
        ...
        param.data[3:3+skel_info[self.kps_convention]['n_kps'] * 3][:3] = angle * np.array([x,y,z], dtype=np.float32)
    ...
    def SolvePose(self, term, param, maxIterTime,hierarchy = False, updateThresh = 1e-4):
        ...
        jCut = 0
        while hier <= hierSize:
            while jCut < skel_info[self.kps_convention]['n_kps'] and skel_info[self.kps_convention]['hierarchy_map'][jCut] <= hier:
                jCut += 1
            for iterTime in range(maxIterTime):
                ...
                for jIdx in range(jCut):
                    nodeWarpsJacobi[:,3 * jIdx:3 * jIdx+3] = rodrigues_jacobi(param.get_pose()[3*jIdx:3*jIdx+3]).T
                ...
                for djIdx in range(jCut):
                    ...
                    for dAxis in range(3):
                        ...
                        if djIdx != 0:
                            dChainWarps[:,4 * djIdx:4 * djIdx+4] = np.matmul(chainWarps[:,4 * skel_info[self.kps_convention]['joint_parent'][djIdx]:4 * skel_info[self.kps_convention]['joint_parent'][djIdx]+4], dChainWarps[:,4*djIdx:4*djIdx+4])

                        for jIdx in range(djIdx+1, jCut):
                            prtIdx = skel_info[self.kps_convention]['joint_parent'][jIdx]
                ...
                if term.wJ2d > 0: 
                    for view in range(int(term.projs.shape[1]/4)):
                        j2dTarget = term.j2dTarget[:,view*skel_info[self.kps_convention]['n_kps']:view*skel_info[self.kps_convention]['n_kps']+skel_info[self.kps_convention]['n_kps']]
                ...
                param.data[:3+skel_info[self.kps_convention]['n_kps'] * 3][:3 + 3 * jCut] += delta
    ...
    def SolveShape(self,term, param, maxIterTime, updateThresh=1e-4):
        for iterTime in range(maxIterTime):
            # calc status
            jBlend = self.CalcJBlend(param)
            ATA = np.zeros((skel_info[self.kps_convention]['shape_size'], skel_info[self.kps_convention]['shape_size']),dtype=np.float32)
            ATb = np.zeros(skel_info[self.kps_convention]['shape_size'],dtype=np.float32)
            ...
            if term.wJ3d > 0 or term.wJ2d > 0:
                ...
                jFinal = self.CalcJFinal_1(chainWarps)
                jointJacobi = np.zeros((3 * skel_info[self.kps_convention]['n_kps'], skel_info[self.kps_convention]['shape_size']),dtype=np.float32)
            ...
            if term.wTemporalShape > 0:
                ATA += term.wTemporalShape * np.identity(skel_info[self.kps_convention]['shape_size'],dtype=np.float32)
                ATb += term.wTemporalShape * (term.paramPrev.get_shape() - param.get_shape())
            
            if term.wSquareShape > 0:
                ATA += term.wSquareShape * np.identity(skel_info[self.kps_convention]['shape_size'],dtype=np.float32)
                ATb -= term.wSquareShape * param.get_shape()
            
            if term.wRegularShape > 0:
                ATA += term.wRegularShape *np.identity(skel_info[self.kps_convention]['shape_size'],dtype=np.float32)
        ...
        for pid, corr_id in enumerate(skels2d):
            ...
            if pid < prevCnt:
                ...
                if not info.shapeFixed:
                    ...
                    corrCnt = np.zeros(skel_info[self.kps_convention]['n_kps'],dtype=np.float32)
                    jConfidence = np.ones(skel_info[self.kps_convention]['n_kps'],dtype=np.float32)
                    for view in range(int(self.projs.shape[1] / 4)):
                        corrCnt += ((poseTerm.j2dTarget[:,view * skel_info[self.kps_convention]['n_kps']:view * skel_info[self.kps_convention]['n_kps']+skel_info[self.kps_convention]['n_kps']][2].T > 0).astype(np.int))

                    for jIdx in range(skel_info[self.kps_convention]['n_kps']):
                        if corrCnt[jIdx] <= 1:
                            jConfidence[jIdx] = 0
                            for view in range(int(self.projs.shape[1] / 4)):
                                poseTerm.j2dTarget[:,view * skel_info[self.kps_convention]['n_kps'] + jIdx] = 0
                ...
                # alloc new person
                if sum(skel[3]> 0) >= self.min_triangulate_cnt:
                    self.m_skelInfos.append((corr_id, SkelInfo(self.kps_convention)))
