...
class FourdMatching(BaseMatching):
    def __init__(self,
        ...
        self.m_epiEdges = {i:{j:{k:-1 for k in range(self.n_views)} for j in range(self.n_views)} for i in range(self.n_joints)}
        self.m_tempEdges = {i:{j:-1 for j in range(self.n_views)} for i in range(self.n_joints)}
        self.m_jointRays = {i:{j:[] for j in range(self.n_joints)} for i in range(self.n_views)}
        self.m_boneNodes = {i:{j:[] for j in range(self.n_views)} for i in range(self.n_pafs)}
        self.m_boneEpiEdges = {i:{j:{k:[] for k in range(self.n_views)} for j in range(self.n_views)} for i in range(self.n_pafs)}
        self.m_boneTempEdges = {i:{j:[] for j in range(self.n_views)} for i in range(self.n_pafs)}
        self.m_assignMap = {i:{j:[] for j in range(self.n_joints)} for i in range(self.n_views)}
    ...
    def cal_bone_nodes(self):
        for paf_id in range(self.n_pafs):
            ...
            for view in range(self.n_views):
                ...
                for joint1_candidate in range(len(self.kps2d_paf[view]['joints'][joint1])):
                    for joint2_candidate in range(len(self.kps2d_paf[view]['joints'][joint2])):
            ...
            for view in range(self.n_views):
                ...
                for pid in range(len(temp)):
                    for node_candidate in range(len(nodes)):
                        ...
                        tempdist = []
                        for i in range(2):
                            tempdist.append(self.m_tempEdges[joint_pair[i]][view][pid][node[i]])
                        if min(tempdist) > 0:
    ...
    def initialize(self):
        for joint_id in range(self.n_joints):
            for view in range(self.n_views):
                self.m_assignMap[view][joint_id] = np.full(len(self.kps2d_paf[view]['joints'][joint_id]), -1)
                ...
                if index >= 0 and pick[index]  >= len(available_node[index][index]):
                    ...
                    if index == 0:
                        ...
                        if pick[self.n_views - 1] > 0:
                            ...
                            tempEdge = self.m_boneTempEdges[paf_id][self.n_views - 1]
    ...
    def assign_top_clique(self):
        ...
        if clique.proposal[self.n_views] != -1:
            ...
            if sum(voting.fstCnt) == 0:
                ...
                if allocFlag():
                    person = np.full((self.n_joints, self.n_views), -1)
                    ...
                    if clique.proposal[view] >= 0:
                        ...
                        if assign_id[0] == master_id and assign_id[1] == master_id:
                            ...
                            if person[unassignj_id, view] == -1 or person[unassignj_id, view] == unassignj_candidata:
                                ...
                                _proposal[view] = clique.proposal[view]
                        elif max(assign_id) == -1 and sum(voting.secCnt) == 0 and (person[joint_pair[0], view] == -1 or person[joint_pair[0], view] == node[0]) and (person[joint_pair[1], view] == -1 or person[joint_pair[1], view] == node[1]):
                            for i in range(2):
                                person[joint_pair[i], view] = node[i]
                                self.m_assignMap[view][joint_pair[i]][node[i]] = master_id
                        else:
                ...
                for index in range(2):
                    while voting.secCnt[index] != 0:
                        ...
                        if self.CheckPersonCompatibility(master_id, slave_id) >= 0:
                            ...
                            voting.vote[voting.fst[index]][index] = voting.vote[voting.sec[index]][index] = 0
    ...
    def cal_clique_score(self, clique):
        ...
        for view1 in range(self.n_views-1):
            ...
            for view2 in range(view1+1, self.n_views):
                if clique.proposal[view2] == -1:
                    continue
                scores.append(self.m_boneEpiEdges[clique.paf_id][view1][view2][clique.proposal[view1], clique.proposal[view2]])
        ...
        if person_id != -1:
            for view in range(self.n_views):
                if clique.proposal[view] == -1:
                    continue
                scores.append(self.m_boneTempEdges[clique.paf_id][view][person_id, clique.proposal[view]])
        ...
        for view in range(self.n_views):
            if clique.proposal[view] == -1:
                continue
            candidata_bone = self.m_boneNodes[clique.paf_id][view][clique.proposal[view]]
            scores.append(self.kps2d_paf[view]['pafs'][clique.paf_id][candidata_bone[0], candidata_bone[1]])
    ...
    def CheckJointCompatibility(self, view, joint_id, candidate, pid):
        ...
        for paf_id in self.m_joint2paf[joint_id]:
            checkJIdx = self.paf_dict[0][paf_id] + self.paf_dict[1][paf_id] - joint_id
            ...
            if self.kps2d_paf[view]['pafs'][paf_id][joint_candidate1, joint_candidate2] > 0:
                checkCnt = checkCnt + 1
            else:
                return -1
        ...
        for i in range(self.n_views):
            if i == view or person[joint_id, i] == -1:
                continue
            if self.m_epiEdges[joint_id][view][i][candidate, int(person[joint_id, i])] > 0:
                checkCnt = checkCnt + 1
            else:
                return -1
        return checkCnt
    ...
    def CheckPersonCompatibility_sview(self, master_id, slave_id, view):
        ...
        for joint_id in range(self.n_joints):
            if master[joint_id, view] != -1 and slave[joint_id, view]!=-1 and master[joint_id, view] != slave[joint_id, view]:
                return -1

        if master_id < len(self.last_multi_kps3d):
            for joint_id in range(self.n_joints):
                if slave[joint_id, view] != -1:
                    if self.m_tempEdges[joint_id][view][master_id, slave[joint_id][view]] > 0:
                        checkCnt = checkCnt + 1
                    else:
                        return -1
        ...
        for paf_id in range(self.n_pafs):
            ...
            for candidate in [[master[self.paf_dict[0][paf_id], view], slave[self.paf_dict[1][paf_id], view]], [slave[self.paf_dict[0][paf_id], view], master[self.paf_dict[1][paf_id], view]]]:
                if min(candidate) >= 0:
            ...
            for view1 in range(self.n_views-1):
                ...
                if candidate1_id != -1:
                    for view2 in range(view1+1, self.n_views):
                        ...
                        if candidate2_id != -1:
                            if self.m_epiEdges[joint_id][view1][view2][candidate1_id, candidate2_id] > 0:
                                checkCnt += 1
                            else:
                                return -1
        return checkCnt
        ...
        for view in range(self.n_views):
            for joint_id in range(self.n_joints):
                if slave[joint_id, view] != -1:
                    ...
                    self.m_assignMap[view][joint_id][slave[joint_id, view]] = master_id
    ...
    def clique2voting(self, clique, voting):
        ...
        for view in range(self.n_views):
            ...
            if index != -1:
                ...
                for i in range(2):
                    assigned = self.m_assignMap[view][self.paf_dict[i][clique.paf_id]][node[i]]
                    if assigned != -1:
