...
class ProjAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4,
        ...
        Modified from DeformableConvV2 and Deformable DETR 
...
class DeformFunction(Function):
    @staticmethod
    def forward(ctx, value, value_spatial_shapes,
...
def deform_core_pytorch(value, value_spatial_shapes,
                        sampling_locations, attention_weights):
    # for debug and test only,
    # need to use cuda version instead
    N_, S_, M_, D_ = value.shape
    _, Lq_, M_, L_, P_, _ = sampling_locations.shape
    value_list = value.split([H_ * W_
                              for H_, W_ in value_spatial_shapes], dim=1)
    ...
    output = (torch.stack(sampling_value_list, dim=-2).
              flatten(-2) * attention_weights).sum(-1).view(N_, M_*D_, Lq_)
