...
class Shelf(KpsDataset):
    ...
    def __init__(self,
        ...
        self.db = self._get_db(osp.join('./data/Shelf/pesudo_gt/', pesudo_gt))
    ...
    def _get_db(self, pesudo_gt_path):
        ...
        db = []
        cameras = self._get_cam_calib_file()
        ...
        for i in self.frame_range:
            for k, cam in cameras.items():
                image = osp.join('Camera' + k, f'img_{i:06d}.png')
                ...
                if self.is_train:
                    for gt_kps3d in gt_voxelpose_infered[image.split('/')[-1]]:
                        if len(gt_kps3d[0]) > 0:
                            ...
                            kps2d_vis = np.ones((len(gt_kps2d), 1))
                            kps2d_vis[np.logical_not(check)] = 0
                            all_gt_kps2d.append(gt_kps2d)
                            all_gt_kps2d_vis.append(
                                np.repeat(
                                    np.reshape(kps2d_vis, (-1, 1)), 2, axis=1))
                else:
    ...
    def evaluate(self, preds, recall_threshold=500):
        ...
        for k, v in limb_group.items():
            limb_person_pcp[k] = np.sum(limb_correct_parts[:, v], axis=-1) \
                                 / (total_parts / 10 * len(v) + 1e-8)

        return \
            actor_pcp, avg_pcp, limb_person_pcp, match_gt / (total_gt + 1e-8)
