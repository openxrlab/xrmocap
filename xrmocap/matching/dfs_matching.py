...
class DFSMatching(BaseMatching):
    ...
    def recursive_step(self, curr_depth: int, mview_kps2d: np.ndarray) -> None:
        for actor_idx, _ in enumerate(mview_kps2d[curr_depth]):
            # if marked or isnan, skip
            if self.mark_mat[curr_depth, actor_idx] > 0 or \
                    (actor_idx > 0 and self.nan_mat[
    ...
    def __call__(
        ...
        mark_mat: np.ndarray,
        nan_mat: np.ndarray,
        init_matched_human_idx: List[int],
        ...
        **kwargs,
        ...
        Args:
            ...
            nan_mat (np.ndarray): Matrix for nan, in shape [n_view, n_person],
                True indicates observation there is np.nan
            init_matched_human_idx (List[int]): Init list of matched
                human index.
            init_matched_human_kps (List[np.ndarray]): Init list of matched
                human kps array.
            We do not use kwargs.
