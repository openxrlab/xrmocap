...
from xrmocap.transform.convention.keypoints_convention import (
    coco, coco_wholebody, crowdpose, gta, h36m, human_data, hybrik,
    instavariety, lsp, mpi_inf_3dhp, mpii, openpose, penn_action, posetrack,
    pw3d, smpl, smplx,
)
...
def convert_kps(
    ...
    dst keypoints definition. Supported conventions by now: coco, smplx,
    ...
    if isinstance(keypoints, torch.Tensor):
        ...
        logger.error('Type of keypoints is neither' +
    ...
    if src == dst:
        if return_mask:
            ret_mask = mask if mask is not None else \
                new_array_func(
                    shape=(keypoints.shape[-2], ),
                    value=1, device_data=keypoints,
                    if_uint8=True)
            return keypoints, ret_mask
        else:
            return keypoints
...
def get_keypoint_idx(name: str,
    ...
    Args:
        ...
        convention (str, optional):
            data type from keypoints_factory. Defaults to 'human_data'.
        approximate (bool, optional):
            Whether approximate mapping is allowed. Defaults to False.
        keypoints_factory (dict, optional):
            A class to store the attributes. Defaults to KEYPOINTS_FACTORY.
