...
- connections: An ndarray for connections, in shape [conn_num, 2], `connections[:, 0]` are start point indice and `connections[:, 1]` are end point indice.  `connections[n, :]`  is `[start_index, end_index]` of the No.n connection.
...
- points:  An ndarray for points, could be None. If not None, it is in shape [point_num, point_dim]. We could use the index record in `connections` to fetch a point.
